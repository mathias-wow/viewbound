...
export function getRandomBackground() {
	...
	if (today >= newYearsStart && today <= newYearsEnd) {
		return {
			url: 'backgrounds/viewbound_new_year.webp',
			author: 'Roven Images',
			location: "Happy New Year's from the Viewbound team!"
		} as Background;
	}
	...
	if (today >= christmasStart && today <= christmasEnd) {
		return {
			url: 'backgrounds/viewbound_christmas.webp',
			author: 'Annie Spratt',
			location: 'Merry Christmas from the Viewbound team!'
		} as Background;
	}
