...
APP_NAME="Viewbound"
INSTALL_DIR="./viewbound"
COMPOSE_FILE_URL="https://raw.githubusercontent.com/seanmorley15/Viewbound/main/docker-compose.yml"
ENV_FILE_URL="https://raw.githubusercontent.com/seanmorley15/Viewbound/main/.env.example"
...
# Check for Viewbound running as a docker container
check_running_container() {
    if docker ps -a --filter "name=viewbound" --format '{{.Names}}' | grep -q "viewbound"; then
        log_error "Viewbound is already running as a Docker container (including stopped or restarting states)."
        echo ""
        echo "Running this installer further can break existing installs."
        echo "Please stop and remove the existing Viewbound container manually before proceeding."
...
prompt_configuration() {
    ...
    echo ""
    echo "Configure the URLs where Viewbound will be accessible."
...
POSTGRES_DB=viewbound
POSTGRES_USER=viewbound
...
start_services() {
    log_info "Starting Viewbound services..."
    echo ""
...
EOF
    ...
    echo -e "   📦 Media Vol:  ${YELLOW}viewbound_media${NC}"
    ...
    echo -e "${BOLD}❤️  Enjoying Viewbound?${NC}"
    echo -e "   Support future development: ${MAGENTA}https://seanmorley.com/sponsor${NC}"
    echo ""

    echo -e "${BOLD}🌍 Adventure awaits — your journey starts now with Viewbound!${NC}"
}
...
print_failure_message() {
    ...
    echo ""
    echo "For support, visit: https://github.com/seanmorley15/Viewbound"
}
