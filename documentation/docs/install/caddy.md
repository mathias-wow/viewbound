...
services:
  caddy:
    ...
    container_name: viewbound-caddy
...
Since all ingress traffic to the Viewbound containsers now travels through Caddy, we can also remove the external ports configuration from those containsers in the `docker-compose.yml`. Just delete this configuration:
...
Your Viewbound should now be up and running.
