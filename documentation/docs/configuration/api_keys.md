...
API keys let you authenticate with Viewbound's REST API without using a session cookie. This is useful for scripts, integrations, or any programmatic access to your data.
...
```bash
curl https://your-viewbound-instance.com/api/adventures/ \
...
response = requests.get("https://your-viewbound-instance.com/api/locations/", headers=headers)
print(response.json())
```
